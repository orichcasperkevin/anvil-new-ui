<template>
	<div class="">
		<section class="container">
			<div class="row mb-4">
				<div class="ml-2 row" v-if="member_info != null">
					<h3><router-link style="text-decoration: none" :to="{name: 'memberList'}">Members </router-link></h3>
					<h3 class="ml-2 mr-2">/</h3>
					<h3 v-for="(data,index) in member_info.member" :key="index">
		...
		<div class="col-lg-2 rounded shadow bg-white" >    
			<h5 class="mt-3 rounded bg-light p-1">Filters</h5>                    
			<nav class="mt-5 nav flex-column nav-pills" id="v-pills-tab" role="tablist" aria-orientation="vertical">
				<a class="nav-link active list-group-item list-group-item-action border-0" id="pill-detail-tab" data-toggle="pill" href="#pill-detail" role="tab" aria-controls="pill-detail" aria-selected="true">
					<i class="fa fa-refresh" aria-hidden="true"></i> detail
				</a>                                                                
				<a class="nav-link list-group-item list-group-item-action border-0" id="pill-groups-tab" data-toggle="pill" href="#pill-groups" role="tab" aria-controls="pill-groups" aria-selected="false" v-on:click = "getMemberGroups()">
					<i class="fa fa-refresh" aria-hidden="true"></i> groups
				</a>                        
				<a class="nav-link list-group-item list-group-item-action border-0" id="pill-contributions-tab" data-toggle="pill" href="#pill-contributions" role="tab" aria-controls="pill-contributions" aria-selected="false" v-on:click = "getMemberFinances()">
					<i class="fa fa-refresh" aria-hidden="true"></i> finances
				</a>
				<a class=" d-none nav-link list-group-item list-group-item-action border-0" id="pill-roles-tab" data-toggle="pill" href="#pill-roles" 
					...
					<i class="fa fa-refresh" aria-hidden="true"></i> roles
				</a> 
		...
		<div class="col">
			<div class="tab-content" id="v-pills-tabContent">
				<!-- member detail -->
				<div class="tab-pane fade show active p-3" id="pill-detail" role="tabpanel" aria-labelledby="pill-detail-tab">                                
						...
						<div class="mt-4 bg-white p-3 rounded shadow-sm">
							<table class="table table-responsive-sm table-borderless" v-if = "groups_selected == true">
								<thead class=""  v-if = "church_groups.response.length > 0">
									...
									<tr class="text-muted" v-for = "(data,index) in church_groups.response" :key="index">
										<td>                                                                
								...
								<div v-if = "groups_selected == true">
									<div class="text-center" v-if = "church_groups.response.length == 0 ">
										<p class="p-3 text-muted">member belongs to none. </p>
									</div>
								</div>
						</div>
				</div>
				...
				<div class=" tab-pane fade" id="pill-contributions" role="tabpanel" aria-labelledby="pill-contributions-tab">
						...
						<section class="tab-content" id="pills-tabContent">
								<div class="tab-pane fade show active bg-white p-3 rounded shadow-sm" id="pills-home" role="tabpanel" aria-labelledby="pills-home-tab" v-if="finances_selected">
									<h3 >Contributions</h3>
									<table class="table table-responsive-sm table-borderless">
										...
										<tbody>
										<tr v-for = "(data,index) in contribution_info.contribution" :key="index">
									...
									<p class="text-center text-muted" v-if="! contribution_info.contribution.length">
											found no contributions by member
									</p>
								</div>
								<div class="tab-pane fade bg-white p-3 rounded shadow-sm" id="pills-profile" role="tabpanel" aria-labelledby="pills-profile-tab" v-if="finances_selected">
									<h3>Pledges</h3>
									<table class="table table-responsive-sm table-borderless">
											<thead>
												<tr>
											...
											<tbody>
												<tr v-for = "(data,index) in pledges_info.pledges" :key="index">
												<td>{{data.project.name}} </td>
									...
									<p class="text-center text-muted" v-if="! pledges_info.pledges.length">
										found no pledges by member.
									</p>
								</div>
								<div class="tab-pane fade bg-white p-3 rounded shadow-sm" id="pills-tithes" role="tabpanel" aria-labelledby="pills-tithes-tab" v-if = "finances_selected">
									<h3>Tithes</h3>                                                    
									<table class="table table-responsive-sm table-borderless">
										<thead>
										<tr>
										...
										<tbody>
										<tr v-for = "(data,index) in tithe_info.tithes" :key="index">
											<td><p class="text-muted">{{humanize(data.amount)}}</p></td>
									...
									<p class="text-center text-muted" v-if="!tithe_info.tithes.length">
										found no tithe by member this month.
									</p>
								</div>
								<div class="tab-pane fade bg-white p-3 rounded shadow-sm" id="pills-offerings" role="tabpanel" aria-labelledby="pills-offerings-tab" v-if = "finances_selected">
										<h3>Offerings </h3>                                                                
										<table class="table table-responsive-sm table-borderless">
											<thead>
											<tr>
											...
											<tbody>
											<tr v-for = "(data,index) in offering_info.offerings" :key="index">
													<td><p class="text-muted">{{humanize(data.amount)}}</p></td>
													<td> {{data.narration}}</td>
													<td>{{$humanizeDate(data.date)}}</td>
											</tr>
										...
										<p class="text-center text-muted" v-if="!offering_info.offerings.length">
		...
		<section>               
			...
			<div class="modal fade" id="deleteMemberModal" tabindex="-1" role="dialog" aria-labelledby="deleteMemberModal" aria-hidden="true">
				...
				<div class="modal-content">
						...
						<h5 class="modal-title border-0" >delete member</h5>
						...
						<div class="modal-body border text-danger">
							...
							<p class="mt-2 small">
								all data about the member will be lost, this is unreversible
							</p>
						</div>
  ...
  methods: {
		...
		getRoleForMember: function (){  
			this.$http.get(this.$BASE_URL +'/api/members/roles-for-member/'+ this.$route.params.id + '/')
				...
				var count = this.roles_for_member.response.length
				if (count > 0){
